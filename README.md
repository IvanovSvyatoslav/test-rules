...
- Один отступ при переносе выражения на следующую строку, знак операции не переносится.
    ...
    Value = x1 + x2 + x3 + x4 +
        x5 + x6;
    ```
- Помещаем скобку, связанную с управляющим оператором, на следующую строку с отступом на том же уровне, что и управляющий оператор. Утверждения внутри скобок отступают на следующий уровень ([Allman style]([allman](https://en.wikipedia.org/wiki/Indentation_style#Allman_style))).
...
### Операторы `&&` и `||`
Чтобы избежать исключений и повысить производительность за счет пропуска ненужных сравнений, используйте && вместо & и || вместо | при выполнении сравнений, как показано в следующем примере.
